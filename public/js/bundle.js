...
var hideAlert = function hideAlert() {
  ...
  if (alert_div) {
    var el_body = alert_div.parentElement;
...
function () {
  ...
  regeneratorRuntime.mark(function _callee(email, password) {
    var _ref2, _ref2$data, data, status, message;
...
var displayMaps = function displayMaps(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoicmljaGNvZGUiLCJhIjoiY2tndzR2NXkzMDZzZzJ5cjFvNzUzanJscSJ9.7SlYFosXiBSTcJnD4Eye6g';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/richcode/ckgw4zrfu0v4819o6tpgz7v34',
    zoom: 10,
    scrollZoom: false // center: [-118.660343, 34.241828],
    // interactive: false
...
function () {
  var _ref3 = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee2(passwordCurrent, password, passwordCheck) {
    var _ref4, status, message;
...
// DOM Elements
var loginForm_form = document.querySelector('.login-form .form');
var userData_form = document.querySelector('.form-user-data');
var userPassword_form = document.querySelector('.form-user-password');
var logout_button = document.querySelector('.nav__el--logout');
var mapbox_div = document.querySelector('#map'); // DELEGATION

if (mapbox_div) {
  var locations = JSON.parse(mapbox_div.dataset.locations);
  (0, _mapbox.displayMaps)(locations);
} // Login


if (loginForm_form) {
  var _loginForm_form$query = loginForm_form.querySelectorAll('.form__input'),
      _loginForm_form$query2 = _slicedToArray(_loginForm_form$query, 2),
      email_input = _loginForm_form$query2[0],
      password_input = _loginForm_form$query2[1];

  loginForm_form.addEventListener('submit', function (e) {
    e.preventDefault();
    (0, _login.login)(email_input.value, password_input.value);
  });
} // Logout


if (logout_button) {
  logout_button.addEventListener('click', _login.logout);
}
...
if (userData_form) {
  userData_form.addEventListener('submit', function (e) {
    ...
    var _userData_form$queryS = userData_form.querySelectorAll('.form__input'),
        _userData_form$queryS2 = _slicedToArray(_userData_form$queryS, 2),
...
if (userPassword_form) {
  userPassword_form.addEventListener('submit', function (e) {
    e.preventDefault();

    var _userPassword_form$qu = userPassword_form.querySelectorAll('.form__input'),
        _userPassword_form$qu2 = _slicedToArray(_userPassword_form$qu, 3),
        currentPassword_input = _userPassword_form$qu2[0],
        newPassword_input = _userPassword_form$qu2[1],
        confirmPassword_input = _userPassword_form$qu2[2];

    (0, _updateSettings.updateUserPassword)(currentPassword_input.value, newPassword_input.value, confirmPassword_input.value);
  });
}
